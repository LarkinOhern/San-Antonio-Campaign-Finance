---
title: "All District Contribution Analysis"
...
for(i in seq_along(files)){
        all_dist_contribs[[i]]<-read.csv(files[[i]])
}
str(all_dist_contribs)

campaign_finance<-bind_rows(all_dist_contribs) %>% as_tibble()

campaign_finance1<-na.omit(campaign_finance) %>% as_tibble() ###na.omit() to drop empty rows created by the two line adresses
...
campaign_finance2<-campaign_finance1 %>% ###this will be our primary tibble going forward
        ...
        separate(contributor, into=c("last_name","first_name"), sep=",") %>% 
...
adress<-adress[-1255]###somehowe the being PAC FEC number got in the adress slot 

zipcode<-(str_subset(campaign_finance1a$first_name,"\\d"))###there are 5 extra entries...

zipcode1<-trimws(zipcode)
...
state<-str_extract(zipcode1, "[:alpha:].") 

state1<-str_subset(state,"[:upper:][:upper:]")
