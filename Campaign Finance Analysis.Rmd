---
title: 'Following the Money: A Reproducible Analysis of Campaign Finance Reports in
  the 2017 San Antonio,TX Mayoral Race'
...
raw_data<-"sa_mayor_race_campaing_finance_data.csv"###insert file name here
...
zipcode1<-as_tibble(trimws(c(zipcode,"NA"))) #for some reason I have one less zipcode than adress, so I've added an NA to get the lenths to match.  Not sure what a better solution would be.
...
While there is plenty of attention on the impact of political contributions on state and national
elections, there is a dearth of coverage for local races. My goal is to develop a reproducible
analysis that can be quickly re-run as new reports become available and can be used for races across
the state of Texas.
...
I'm using the ongoing 2017 San Antonio mayoral race to develop the analysis.  The campaign finance
reports are available here:(get web link later).  The first report I analyzed was released **`r campaign_finance2$report_date[1]`**.  At that time there were **`r num_candidates`** candidates: **`r cand1`**, 
**`r cand2`** and **`r cand3`**.  The report included data on **`r num_contributions`** contributions from **`r num_indv_contributors`** individual contributors and **`r num_pacs`** Political Action Committees worth **$`r total_contributions`** recorded between **`r first_contribution`** and **`r last_contribution`**.
...
There is much greater variation in the size of individual contributions, especially at the low end while PAC giving was more uniform.  The graph is a bit misleading as individual contributors vastly outnumber PACs in this report.
...
Mayor Ivy has raised significantly more than her opponents both locally and out of state.  She has not only received more contributions, but her average contribution value is $200 greater than the either other candidate. As you can see if the table below she is killing it with max level donors ($1000) especially PACs.

```{r, echo=FALSE}

kable(maxers,digits=2, format.args=list(big.mark=","))

kable(pac_giving,digits=2, format.args=list(big.mark=","))
...
In-kind contributions appear insignificant compared to monetary contributions, though Mayor Ivy has received nearly $11,000 worth of in-kind contributions.
